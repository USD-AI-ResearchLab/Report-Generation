...
import time
import json
from pycocoevaluation.bleu.bleu import Bleu
from pycocoevaluation.cider.cider import Cider
from pycocoevaluation.meteor.meteor import Meteor
from pycocoevaluation.rouge.rouge import Rouge
...
def _gt_fin_generator(para):
    ...
    return gt_fin
def _generate_imp_fin_dict_train(predicted_imps_lst, predicted_fins_lst, image_ids_lst, args):
    pre_imp_dict = {}
    pre_fin_dict = {}
    pre_imp_fin_dict = {}
    gt_imp_dict = {}
...
def compute_metrics_train(predicted_imps_lst, predicted_fins_lst, image_ids_lst, args):
    gt_imp_dic, pre_imp_dic, gt_fin_dic, pre_fin_dic, gt_imp_fin_dic, pre_imp_fin_dic = \
        _generate_imp_fin_dict_train(predicted_imps_lst, predicted_fins_lst, image_ids_lst, args)
    print(30*"#")
    if args.imp_fin_only:
        imp_fin_bleu, imp_fin_meteor, imp_fin_rouge, imp_fin_cider = _define_metrics(gt_imp_fin_dic, pre_imp_fin_dic)
        print('Train Impression + Finding: bleu = %s, meteor = %s, rouge = %s, cider = %s' % (
            imp_fin_bleu, imp_fin_meteor, imp_fin_rouge, imp_fin_cider))
        ...
        imp_fin_bleu, imp_fin_meteor, imp_fin_rouge, imp_fin_cider = _define_metrics(gt_imp_fin_dic, pre_imp_fin_dic)
        print(
            'Train Impression: bleu = %s, meteor = %s, rouge = %s, cider = %s' % (imp_bleu, imp_meteor, imp_rouge, imp_cider))
        print('Train Finding: bleu = %s, meteor = %s, rouge = %s, cider = %s' % (fin_bleu, fin_meteor, fin_rouge, fin_cider))
        print('Train Impression + Finding: bleu = %s, meteor = %s, rouge = %s, cider = %s' % (
            imp_fin_bleu, imp_fin_meteor, imp_fin_rouge, imp_fin_cider))
