...
from metrics import compute_metrics, generate_text_file, compute_metrics_train
from utils.logger import create_logger
from IUdata.vocab_creation import JsonReader, Vocabulary
...
def train_net(num_run, logger, args):
    ...
    val_data_loader = get_loader(args.image_dir, args.eval_json_dir,
                             vocab, train_transform, args.eval_batch_size,
                             args.num_workers, args.max_impression_len,
                             args.max_sen_num, args.max_single_sen_len,
                             shuffle=True)
    ...
    if os.path.exists('train_imp_lss.txt'): os.remove('train_imp_lss.txt')
    if os.path.exists('train_imp_lss.txt'): os.remove('train_fin_lss.txt')
    if os.path.exists('val_imp_lss.txt'): os.remove('val_imp_lss.txt')
    if os.path.exists('val_imp_lss.txt'): os.remove('val_fin_lss.txt')
    ...
    for epoch in range(args.epochs):
        for i, (frontal_imgs, _, impressions, findings, imp_lengths, fin_lengths) in enumerate(data_loader):
            ...
            if i % args.log_step == 0:
                ...
                with open('train_imp_lss.txt', 'a') as f:
                  f.write('{},{},{},{},{},{}'.format(epoch + 1, args.epochs, i, total_step, imp_loss.item(), np.exp(imp_loss.item())))
                  f.write('\n')
                ...
                with open('train_fin_lss.txt', 'a') as f:
                  f.write('{},{},{},{},{},{}'.format(epoch + 1, args.epochs, i, total_step, fin_loss.item(), np.exp(fin_loss.item())))
                  f.write('\n')


        if (epoch + 1) % args.log_metrics_step == 0:

            pre_imps_lst, pre_fins_lst, img_id_lst = in_training(image_encoder, impression_decoder,
                                                                       finding_decoder, vocab, args)

            imp_fin_bleu, imp_fin_meteor, imp_fin_rouge, imp_fin_cider = compute_metrics_train(pre_imps_lst, pre_fins_lst,
                                                                                         img_id_lst, args)
            ...
            print("VALIDAION LOSS")
            print(30*"#")
            for i, (frontal_imgs, _, impressions, findings, imp_lengths, fin_lengths) in enumerate(val_data_loader):
                frontal_imgs = frontal_imgs.to(device)
                impressions = impressions.to(device)
                findings = findings.to(device)
                # impression
                ...
                if i % args.log_step == 0:
                    ...
                    with open('val_imp_lss.txt', 'a') as f:
                      f.write('{},{},{},{},{},{}'.format(epoch + 1, args.epochs, i, total_step, imp_loss.item(), np.exp(imp_loss.item())))
                      f.write('\n')
                    ...
                    with open('val_fin_lss.txt', 'a') as f:
                      f.write('{},{},{},{},{},{}'.format(epoch + 1, args.epochs, i, total_step, fin_loss.item(), np.exp(fin_loss.item())))
                      f.write('\n')
...
def in_training(image_encoder, impression_decoder, finding_decoder, vocab, args):
    # testing dataset loader
    test_transforms = transforms.Compose([
        transforms.Resize(args.resize_size),
        transforms.CenterCrop(args.crop_size),
        ...
        transforms.Normalize((0.485, 0.456, 0.406),
                             (0.229, 0.224, 0.225))])
    eval_data_loader = get_loader(args.image_dir, args.json_dir,
                                  vocab, test_transforms, args.eval_batch_size,
                                  args.num_workers, args.max_impression_len,
                                  args.max_sen_num, args.max_single_sen_len, shuffle=False)
    vocab_size = len(vocab)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vocab_path', type=str, default='IUdata/data_vocab_threshold_0.pkl', help='path for vocabulary wrapper')
